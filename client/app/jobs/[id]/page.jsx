...
const fetchJob = async (id) => {
	let job = await fetch(`http://localhost:1337/api/jobs/${id}?populate=*`)
	return job.json()
}

const DetailedJobs = async ({ params: { id } }) => {
	...
	return ( 
		...
		<article className="px-4 py-5 mx-auto max-w-7xl">
			<div className="w-full mx-auto mb-12 text-left md:w-3/4 lg:w-1/2">
				<p className="mt-6 mb-2 text-xs font-semibold tracking-wider uppercase text-primary">{job.data.attributes.categories.data[0].attributes.type + " LEVEL"}</p>
				<h1 className="mb-3 text-3xl font-bold leading-tight text-gray-900 md:text-4xl">
				{job.data.attributes.vacancy}
				</h1>
				...
				<div className="avatar">
					...
					src={`http://localhost:1337${job.data.attributes.image.data.attributes.url}`}
					alt={"Photo of " + job.data.attributes.recruiter} />
				</div>
				<div className="ml-4">
					<p className="font-semibold text-gray-800 text-md">{job.data.attributes.recruiter}</p>
					<p className="text-sm text-gray-500">{job.data.attributes.publishedAt.substring(0, Max_DATE_LENGHT)}</p>
					<p className="text-sm text-gray-500">{job.data.attributes.createdAt.substring(16, Max_TIME_LENGHT)}</p>
				</div>
				</a>
			</div>
